package com.herbert.rnimageresizer;
...
class ImageResizer {
    ...
    /**
     * Save the given bitmap in a directory. Extension is automatically generated
     ...
     * Use content resolver to get real path if direct path doesn't return valid
     * file.
     */
    ...
    /**
     * Compute the inSampleSize value to use to load a bitmap. Adapted from
    ...
    /**
     * Load a bitmap either from a real file or using the {@link ContentResolver} of
     * the current {@link Context} (to read gallery images for example).
     */
    private static Bitmap loadBitmap(Context context, String imagePath, BitmapFactory.Options options)
            throws IOException {
     ...
     */
    private static Bitmap loadBitmapFromFile(Context context, String imagePath, int newWidth, int newHeight)
            throws IOException {
    ...
    /**
     * Loads the bitmap resource from a base64 encoded jpg or png. Format is as
     * such: png: 'data:image/png;base64,iVBORw0KGgoAA...' jpg:
     ...
     */
    public static String createResizedImage(Context context, String imagePath, int newWidth, int newHeight,
            Bitmap.CompressFormat compressFormat, int quality, int rotation, String outputPath) throws IOException {
        Bitmap sourceImage = null;

        // If the BASE64_PREFIX is absent, load bitmap from a file. Otherwise, load from
        // base64.
        ...
        String resizedImagePath = ImageResizer.saveImage(rotatedImage, path, Long.toString(new Date().getTime()),
