...
class RNPixelColorModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void getPixelRGBAPolarOfImage(final String imageName, final double angle, final double radius,
    ...
    @ReactMethod
    public void findAngleOfNearestColor(final String imageName, final double minAngle, final double maxAngle,
            final double radius, final ReadableArray targetColor, final Callback callback) {
        try {
            final Bitmap image = loadImage(imageName);
            final double width = image.getWidth();
            final double height = image.getHeight();
            ...
            while (angle <= maxAngle) {
                ...
                final double distance = pow(targetRed - red, 2) + pow(targetGreen - green, 2)
